...
2. **TLS配置问题**
   ...
   - 如果设置了`quic=1`但禁用TLS，系统会自动启用`tls=1`
   - 生产环境使用`tls=2`和有效证书
   - 检查QUIC连接的证书有效性

3. **客户端-服务器QUIC不匹配**
   - 服务器和客户端必须使用相同的`quic`设置
   - 带`quic=1`的服务器需要带`quic=1`的客户端
   - 带`quic=0`的服务器需要带`quic=0`的客户端
