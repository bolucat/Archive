...
func (l *LwIP) processPacket() error {
	...
	if err != nil {
		return newError("failed to write packet to LWIP").Base(err)
	}
	return nil
}
...
func (l *LwIP) ReceiveTo(conn core.UDPConn, data []byte, addr *net.UDPAddr) error {
	srcAddr := conn.LocalAddr().String()
	src, err := net.ParseDestination(fmt.Sprint("udp:", srcAddr))
	if err != nil {
		logrus.Warn("[UDP] parse source address ", srcAddr, " failed: ", err)
		return err
	}
	dstAddr := addr.String()
	dst, err := net.ParseDestination(fmt.Sprint("udp:", dstAddr))
	if err != nil {
