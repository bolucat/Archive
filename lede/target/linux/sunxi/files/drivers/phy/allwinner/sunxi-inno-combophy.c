				     ...
				     struct sunxi_combphy *combphy)
{
	...
	/* combo phy use sel */
	ret = of_property_read_u32(np, KEY_PHY_USE_SEL, &val);
	if (ret)
		return dev_err_probe(dev, ret, "failed to get phy_use_sel\n");
	if (val > PHY_USE_BY_PCIE_USB3_U2)
		return dev_err_probe(dev, -EINVAL, "invalid phy_use_sel\n");
	combphy->user = val;

	combphy->phyclk_ref = devm_clk_get(&pdev->dev, "phyclk_ref");
	if (IS_ERR(combphy->phyclk_ref))
		return dev_err_probe(dev, PTR_ERR(combphy->phyclk_ref),
				     "failed to get phyclk_ref\n");

	combphy->reset = devm_reset_control_get(dev, "phy_rst");
	if (IS_ERR(combphy->reset))
		return dev_err_probe(dev, PTR_ERR(combphy->reset),
				     "failed to get reset control\n");

	if (combphy->drvdata->need_noppu_rst) {
		combphy->noppu_reset = devm_reset_control_get(dev, "noppu_rst");
		if (IS_ERR(combphy->noppu_reset))
			return dev_err_probe(dev, PTR_ERR(combphy->noppu_reset),
					     "failed to get noppu_reset control\n");
	}
	if (combphy->user == PHY_USE_BY_PCIE || combphy->user == PHY_USE_BY_PCIE_USB3_U2) {
		combphy->refclk_par = devm_clk_get(&pdev->dev, "refclk_par");
		if (IS_ERR(combphy->refclk_par))
			return dev_err_probe(dev, PTR_ERR(combphy->refclk_par),
					     "failed to get refclk_par\n");

		ret = clk_set_parent(combphy->phyclk_ref, combphy->refclk_par);
		if (ret)
			return dev_err_probe(dev, ret, "failed to set refclk parent\n");
	}

	if (combphy->drvdata->has_cfg_clk) {
		combphy->phyclk_cfg = devm_clk_get(&pdev->dev, "phyclk_cfg");
		if (IS_ERR(combphy->phyclk_cfg))
			return dev_err_probe(dev, PTR_ERR(combphy->phyclk_cfg),
					     "failed to get phyclk_cfg\n");

		combphy->cfgclk_par = devm_clk_get(&pdev->dev, "cfgclk_par");
		if (IS_ERR(combphy->cfgclk_par))
			return dev_err_probe(dev, PTR_ERR(combphy->cfgclk_par),
					     "failed to get cfgclk_par\n");

		ret = clk_set_parent(combphy->phyclk_cfg, combphy->cfgclk_par);
		if (ret)
			return dev_err_probe(dev, ret, "failed to set cfgclk parent\n");
	}

	if (combphy->drvdata->has_pcie_axi_clk) {
		combphy->phy_axi = devm_clk_get(&pdev->dev, "pclk_axi");
		if (IS_ERR(combphy->phy_axi))
			return dev_err_probe(dev, PTR_ERR(combphy->phy_axi),
					     "failed to get pclk_axi\n");

		combphy->phy_axi_par = devm_clk_get(&pdev->dev, "pclk_axi_par");
		if (IS_ERR(combphy->phy_axi_par))
			return dev_err_probe(dev, PTR_ERR(combphy->phy_axi_par),
					     "failed to get pcie_axi_par\n");

		ret = clk_set_parent(combphy->phy_axi, combphy->phy_axi_par);
		if (ret)
			return dev_err_probe(dev, ret, "failed to set parent\n");
	}

	if (combphy->drvdata->has_phy_mbus_clk) {
		combphy->phy_mclk = devm_clk_get(&pdev->dev, "phy_mclk");
		if (IS_ERR(combphy->phy_mclk))
			return dev_err_probe(dev, PTR_ERR(combphy->phy_mclk),
					     "failed to get phy_mclk\n");
	}

	if (combphy->drvdata->has_phy_ahb_clk) {
		combphy->phy_hclk  = devm_clk_get(&pdev->dev, "phy_hclk");
		if (IS_ERR(combphy->phy_hclk))
			return dev_err_probe(dev, PTR_ERR(combphy->phy_hclk),
					     "failed to get phy_hclk\n");
	}
	...
	ret = of_property_read_u32(np, KEY_PHY_REFCLK_SEL, &val);
	if (ret)
		return dev_err_probe(dev, ret, "failed to get phy_refclk_sel\n");
	if (val > EXTER_DIF_REF_CLK)
		return dev_err_probe(dev, -EINVAL, "invalid phy_refclk_sel\n");
	combphy->ref = val;

	/* select ic supply */
	combphy->select3v3_supply = devm_regulator_get_optional(&pdev->dev, "select3v3");
	if (IS_ERR(combphy->select3v3_supply)) {
		ret = PTR_ERR(combphy->select3v3_supply);
		if (ret != -ENODEV)
			return dev_err_probe(dev, ret,
					     "failed to get select3v3 supply\n");
...
err_subsystem:
	...
	return dev_err_probe(dev, ret, "failed to register combo PHY\n");
}
