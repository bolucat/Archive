...
import { triggerBookScanIfDue } from './bookLibraryBootstrap'
...
export async function promptAutoScanForUser(userId: string, userLabel: string): Promise<void> {
  ...
  inflightPrompt = (async () => {
    try {
      ...
      if (choice.book) partial.uiLibraryAutoScanBook = true
      ...
      const bookDisabled = new Set(setting.uiLibraryAutoScanBookDisabledUsers || [])
      ...
      if (choice.book) bookDisabled.delete(userId)
      else bookDisabled.add(userId)
      ...
      partial.uiLibraryAutoScanBookDisabledUsers = Array.from(bookDisabled)
      ...
      if (choice.book) {
        triggerBookScanIfDue(true).catch((e) => DebugLog.mSaveWarning('post-login book scan: ' + (e as Error).message))
      }
    } catch (e) {
...
function askUser(userLabel: string): Promise<ScanChoice> {
  return new Promise<ScanChoice>((resolve) => {
    ...
    const choice: ScanChoice = {
      ...
      book: setting.uiLibraryAutoScanBook === true
    }
    Modal.confirm({
      title: '开启后台静默扫描？',
      okText: '保存设置',
      ...
      content: () => h('div', { style: 'min-width: 360px; line-height: 1.7' }, [
        h('div', { style: 'color: var(--color-text-2); font-size: 13px; margin-bottom: 10px' }, [
          `检测到新账号「${userLabel}」登录成功，是否开启后台静默扫描以收录该账号下的音乐、视频与书籍？`
        ]),
        h('div', { style: 'display: flex; flex-direction: column; gap: 8px' }, [
          ...
          h('label', { style: 'cursor: pointer; user-select: none' }, [
            h('input', {
              ...
              style: 'margin-right: 6px; vertical-align: middle',
              onChange: (e: Event) => {
                choice.book = (e.target as HTMLInputElement).checked
              }
            }),
            '书籍库：每次启动 App 静默扫描电子书文件'
          ]),
          h('div', { style: 'color: var(--color-text-3); font-size: 12px; margin-top: 4px' },
            '稍后可在「设置 → 网盘 → 媒体库后台自动扫描」中随时调整。')
        ])
      ]),
      onOk: () => resolve({ music: choice.music, video: choice.video, book: choice.book }),
      onCancel: () => resolve({ music: false, video: false, book: false })
    })
  })
}
