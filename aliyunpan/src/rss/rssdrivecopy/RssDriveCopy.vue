...
const handleLeftTreeSelect = (selectkeys: any) => {
  ...
  TreeState.LeftCheckedKeys = []
  if (!TreeState.LeftInfo.user_id) {
    message.error('未选择需要被复制的盘')
    return
  }
...
const handleRightTreeSelect = (selectkeys: any) => {
  ...
  if (!TreeState.RightInfo.user_id) {
    message.error('未选择需要复制到的盘')
    return
  }
  ...
  <div class='scanfill rightbg'>
    ...
    <div class='settingcard scanauto' style='padding: 4px; margin-top: 4px'>
      <a-row justify='space-between' align='center'
             style='margin: 12px; height: 28px; flex-grow: 0; flex-shrink: 0; overflow: hidden'>
        ...
        <a-button v-if='copyLoading' size='small' tabindex='-1' style='margin-right: 12px' @click='handleReset'>取消
        </a-button>
        <a-button type='primary' size='small' tabindex='-1' :loading='copyLoading' @click='handleCopy'>开始复制
        </a-button>
      </a-row>
