...
import { approveWorkspacePlan, archiveWorkspaceTask, cancelWorkspaceTask, getWorkspaceTask, listWorkspaceTasks, rejectWorkspacePlan, restoreWorkspaceTask, resumeWorkspaceTask, updateWorkspacePlanSelection } from '../services/workspaceAgent/client'
...
function selectTask(task: WorkspaceTaskView) { activeTaskId.value = task.id; syncSelectedActions(task) }
...
onMounted(async () => { await Promise.all([loadDrives(), refresh()]) })
</script>
    ...
    <main class="workspace-agent__main">
      <template v-if="activeTask">
        ...
        <section class="workspace-agent__plan-panel">
          <div class="workspace-agent__plan-heading"><div><h2>{{ activeTask.plan ? '待确认计划' : '计划状态' }}</h2><span>{{ activeTask.plan ? '基于已验证的证据生成' : activeTask.summary || '尚未生成可执行计划' }}</span></div><strong v-if="reclaimedBytes">预计释放空间：{{ humanSize(reclaimedBytes) }}</strong></div>
