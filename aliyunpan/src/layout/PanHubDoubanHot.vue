...
const props = defineProps<{ apiBase: string; cachedItems?: Record<string,{items:any[];hasMore:boolean}> }>()
...
const currentPage=ref(1);const hasMore=ref(false);

async function fetchData(categoryId:string,page:number,append:boolean,force=false){
  const cached = props.cachedItems?.[categoryId];
  if (!force && cached?.items?.length && page === 1 && !append) {
    items.value = cached.items; hasMore.value = cached.hasMore ?? false; loading.value = false; return
  }
...
function selectCategory(id:string,force=false){activeCategory.value=id;items.value=[];currentPage.value=1;hasMore.value=false;fetchData(id,1,false,force)}
...
onMounted(()=>{fetchData(activeCategory.value,1,false)})
</script>

<template>
  <div class="ph-douban">
    <div class="ph-douban-header"><Film :size="16" :stroke-width="1.5" class="ph-douban-header-icon" /><span>豆瓣影视</span><button v-if="!loading&&items.length>0" class="ph-douban-refresh" type="button" @click="selectCategory(activeCategory,true)"><RefreshCw :size="13" :stroke-width="1.5" /></button></div>
    ...
    <div v-else class="ph-douban-grid">
      <div v-for="item in items" :key="item.id||item.title" class="ph-douban-card" @click="emit('select',extractTerm(item.title))">
        ...
        <div class="ph-douban-title">{{item.title}}</div>
      </div>
    </div>
    <button v-if="hasMore&&!loadingMore" class="ph-douban-more" type="button" @click="loadMore">加载更多...</button>
...
.ph-douban-more{display:block;margin:14px auto 0;padding:4px 12px;font-size:12px;color:var(--color-text-4);background:transparent;border:0;border-radius:999px;cursor:pointer}
.ph-douban-more:hover{color:var(--color-text-2);background:var(--color-fill-1)}
