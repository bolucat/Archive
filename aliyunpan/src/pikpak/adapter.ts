...
export const listPikPakItems = async (userId: string, driveId: string, dirId: string, includeFiles: boolean, pageToken = '', strict = false) => {
  ...
  const { items, nextPageToken } = await apiPikPakFileList(userId, parentId, 100, pageToken, isTrash, strict)
