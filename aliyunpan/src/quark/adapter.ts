...
export const listQuarkItems = async (userId: string, driveId: string, dirId: string, includeFiles: boolean) => {
  ...
  const result = isSearch
    ...
    : await apiQuarkFileList(userId, parentId, 200)
  ...
  return { items: visibleItems, total: result.total || visibleItems.length }
}
