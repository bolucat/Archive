...
# TODO: Is Android support at all?
# if [ "$target_os" = 'android' ]; then
...
if [ "$ARCH" = 'Linux' ]; then
  static_libs="./$static_lib_name $static_libs -lm"
  # Non-Openwrt linux seems to require this.
...
if [ "$target_cpu" = 'mipsel' -o "$target_cpu" = 'mips64el' ]; then
  ...
  # Segfaults if built with PIE. TODO: Find out why.
  buildmode_flag=
fi
